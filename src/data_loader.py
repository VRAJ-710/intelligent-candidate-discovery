...
def load_candidates(path: str) -> pd.DataFrame:
    ...
    print(f"Loading {len(lines):,} candidates...")
    ...
    for line in tqdm(lines):
        ...
        skill_advanced = [s["name"].lower() for s in skills
                          if s.get("proficiency") == "advanced"]
        total_endorsements = sum(s.get("endorsements", 0) for s in skills)

        # --- Flatten career ---
        all_descriptions = " ".join(c.get("description", "") for c in career)
        all_titles = " ".join(c.get("title", "") for c in career)
        industries = list(set(c.get("industry", "") for c in career))

        # --- Consulting-only flag (JD explicitly penalizes this) ---
        consulting_firms = {"tcs", "infosys", "wipro", "accenture",
                            "cognizant", "capgemini", "hcl", "mindtree",
                            "tech mahindra"}
        companies_worked = set(
            c.get("company", "").lower() for c in career
        )
        only_consulting = companies_worked.issubset(consulting_firms)

        # --- Best education tier ---
        tier_map = {"tier_1": 1, "tier_2": 2, "tier_3": 3, "tier_4": 4}
        tiers = [tier_map.get(e.get("tier", "tier_4"), 4) for e in education]
        best_tier = min(tiers) if tiers else 4

        # --- Salary sanity (some have min > max, data quirk) ---
        ...
        # --- Build full text for semantic matching ---
        full_text = " ".join([
            p.get("headline", ""),
            p.get("summary", ""),
            all_titles,
        ...
        records.append({
            ...
            "advanced_skill_count": len(skill_advanced),
